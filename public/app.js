...
firebase.auth().onAuthStateChanged( (user) => {
    if(user){
        ...
        botones.innerHTML=/*html*/ `
            <button class="btn btn-outline-light" id='CerrarSecion'>Cerrar Sesión</button>
        `  
        ...
        botones.innerHTML = /*html*/`
            <button class="btn btn-outline-light mr-2" id='Acceder'>Acceder</button>
        `
                ...
                if(doc.data().uid===user.uid){
                    Contenido_Protegido.innerHTML += /*html*/ `
                        <div class="d-flex justify-content-end">
                            <span class="badge rounded-pill bg-dark fs-5">${doc.data().texto}</span>
                        </div>
                    `
                }else{
                    Contenido_Protegido.innerHTML += /*html*/`
                        <div class="d-flex justify-content-start">
                            <span class="badge rounded-pill bg-white text-dark fs-5">${doc.data().texto}</span>
                        </div>
                    `
                }
